---
title: "Counterfactuals Explained"
...
Counterfactuals are a "what if" question.  In this case, it is an attempt to answer: *"What if" a different course was taken?*  It takes the model that was trained on the actual courses taken, swaps out the course for the other options, and produces an expected grade.  
...
Then, in order to get the best counterfactual estimates, the predictions should be bounded to be reasonably close to what happened in reality. 

In this case, it is recommended to limit counterfactuals to courses that are less than three distance units away.  This limit is explored below.  (A more thorough approach would be to use the "MatchIt" package to better match the fictitious, counterfactual estimates with similar actual students.)   
...
![](Counterfactuals-Explained_files/figure-html/unnamed-chunk-11-1.png)<!-- -->



![](Counterfactuals-Explained_files/figure-html/unnamed-chunk-12-1.png)<!-- -->




![](Counterfactuals-Explained_files/figure-html/unnamed-chunk-13-1.png)<!-- -->
...
A look at the counterfactuals shows a wide range of estimated grades as well.  The counterfactuals are sorted from the closest to the farthest, with the three-unit cutoff shown as horizontal red line. His actual performance is shown as a circled red 'x'.  
