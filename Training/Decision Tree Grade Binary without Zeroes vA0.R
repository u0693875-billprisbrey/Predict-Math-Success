# Decision Tree Grade Binary without Zeroes vA0
...
# It follows Decision Tree Zeroes vA0, but with these adjustments:
#   - Grades of Zero (E or EU) are removed  
#   - Withdrawals are filtered out  
#   - Binary is divided at 2.7, or between C+ and B-
...
# Combine grades
# ADJUSTED BINARY
firstCourses$grade_binary_adj <- NA
hiFilter <- firstCourses$GRADE %in% c("A", "A-", "B+", "B", "B-" )
loFilter <- firstCourses$GRADE %in% c( "C+", "C", "C-", "D+", "D", "D-", "E", "EU" ) 
firstCourses$grade_binary_adj[hiFilter] <- "hi"
firstCourses$grade_binary_adj[loFilter] <- "low"
...
# Combined filters
cleanData <- firstCourses[cleanFilters & outlierFilters & course_levelFilter & recent_filter & ageFilter & withdrawFilter & zeroFilter,]
...
perClass <- lapply(as.character(2007:2025), function(tYr){
  ...
  missingCourse <- setdiff(cleanData$course[cleanData$class_year == as.numeric(tYr)], data_list[[tYr]][["training"]][["course"]])
  
  if (length(missingCourse) == 0) {
    # no missingCourse values — keep everything
    # missingCourseFilter <- rep(TRUE, nrow(data_list[[tYr]][["testing"]]))
    missingCourseFilter <- rep(TRUE, nrow(cleanData[cleanData$class_year == as.numeric(tYr),]))
  } else {
    # normal case — exclude the missingCourse courses
    # missingCourseFilter <- !data_list[[tYr]][["testing"]][["course"]] %in% missingCourse
    missingCourseFilter <- !cleanData$course[cleanData$class_year == as.numeric(tYr)] %in% missingCourse
  ...
  missingEthnicity <- setdiff(cleanData$ETHNICITY[cleanData$class_year == as.numeric(tYr)],data_list[[tYr]][["training"]][["ETHNICITY"]])
  
  if (length(missingEthnicity) == 0) {
    # no missingCourse values — keep everything
    # missingEthnicityFilter <- rep(TRUE, nrow(data_list[[tYr]][["testing"]]))
    missingEthnicityFilter <- rep(TRUE, nrow(cleanData[cleanData$class_year == as.numeric(tYr),]))
  } else {
    # normal case — exclude the missingCourse courses
    # missingEthnicityFilter <- !data_list[[tYr]][["testing"]][["ETHNICITY"]] %in% missingEthnicity
    missingEthnicityFilter <- !cleanData$ETHNICITY[cleanData$class_year == as.numeric(tYr)] %in% missingEthnicity
  ...
  # predict 
  cleanData$pred[cleanData$class_year == as.numeric(tYr)][missing] <- predict(
    model_list[[tYr]],
    #newData = data_list[[tYr]][["testing"]][missing,]
    newData = cleanData[cleanData$class_year == as.numeric(tYr),][missing,]
  )
  
  return(cleanData[cleanData$class_year == as.numeric(tYr),][missing,])
...
# sampleIndex <- createDataPartition(cleanData$cleanGrade, p = 0.1, list = FALSE)
