...
# Looks like I saved this before I deleted it. 

# At some future time I might want a vQ1, but as of 10 Feb 2026 I decided not to train
# a new "over time" model for grades.  This document has no reason to exist and can be ignored.
...
# It follows vK0 but with these adjustments:
#   It follows the data prep used in vH4, where z-scores and distances are calculated
...
lapply( c(2007:2025),  function(testYear){  # 2007:2025
  ...
  lapply(target_classes,
         ...
         function(target){
           ...
           testData  <- popSample[testIndex, c(target, keepColumns[-which(keepColumns %in% c("cleanGrade", "EMPLID") )])]
           ...
           saveRDS(theData, here::here("Data", "Over Time", paste("Decision Tree", model_version, testYear, target, "Data.rds")))
           ...
           saveRDS(fit, here::here("Models", "Over Time", paste("Decision Tree", model_version,  testYear, target, "model.rds")))
