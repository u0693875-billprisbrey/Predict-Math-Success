# Decision Tree Grades B0

# PURPOSE:  
# This runs various training models with various bucketing of grades.
# Models are xgboost and I'm not sure what my sampling will be.
...
set.seed(123)
subIndex <- createDataPartition(cleanData$cleanGrade, p = 0.02, list = FALSE)
...
lapply(target_classes,
       ...
       function(target){
         ...
         popSample <- cleanData[subIndex, c(target,keepColumns)] |>
  ...
  #     })
         ...
         saveRDS(theData, here::here("Models", paste("Decision Tree vB0", target, "Data.rds")))
         
         
         ## TRAIN ##  
         
         startTime <- Sys.time()
         rep(print(paste("STARTING", target, "AT", startTime,"\n")),3)
         ...
         saveRDS(fit, here::here("Models", paste("Decision Tree vB0", target, "model.rds")))
...
stop("Nothing happens after here")
