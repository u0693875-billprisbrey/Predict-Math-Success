...
# It follows vB0, but with this major adjustment:
# -   It includes the predicted GPA in both training and testing sets 
...
# Build file paths
model_files <- here::here("Models", paste0("Decision Tree ", "vH2", " ", "GRADEGPA", " model.rds"))
...
# Build file paths
data_files <- here::here("Data", paste0("Decision Tree ", "vH2", " ", "GRADEGPA", " Data.rds"))
...
popSample <- wData[ , c(target,keepColumns)] |> # no sampling index 
...
trainData$pred <- predict(model_list[["GRADEGPA"]], trainData)
testData$pred <- predict(model_list[["GRADEGPA"]], testData)

theData <- list(training = trainData, testing = testData)

# stop("Check to here")

saveRDS(theData, here::here("Data", paste("Decision Tree vC0", target, "Data.rds")))
...
saveRDS(fit, here::here("Models", paste("Decision Tree vC0", target, "model.rds")))
