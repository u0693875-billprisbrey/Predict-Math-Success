...
# It follows vJ0, but with this adjustment:
#   ACT test scores are removed
#   A flag indicating ACT test score submission is added
#   A flag indicating SAT test score submission is added  
...
academic_prep <- c(
  ...
  "ACTMATH",
  "ACTSCI", 
  "SATMATH", # including for SAT flag
  "SATVERBAL",
  "SATWRTG"        
)
...
courseClusters <- table(firstCourses$course) |> # table(firstCourses$course[recent_filter])
...
# Longer list for iterating through the years not shown
# > higherVolCourses
...
# TEST FLAGS
firstCourses$ACT_submit <- NA
firstCourses$ACT_submit <- ifelse(is.na(firstCourses$ACTMATH), "not_submit", "submit" )

firstCourses$SAT_submit <- NA
firstCourses$SAT_submit <- ifelse(is.na(firstCourses$SATMATH), "not_submit", "submit" )
...
# Combined filters
cleanData <- firstCourses[cleanFilters & outlierFilters & course_levelFilter & courseFilter  & ageFilter & withdrawFilter,] # & recent_filter
...
lapply(2007:2025,  function(testYear){ 
  ...
  lapply(target_classes,
         ...
         function(target){
           ...
           saveRDS(theData, here::here("Data", "Over Time", paste("Decision Tree vK0", testYear ,target, "Data.rds")))
           
           
           ## TRAIN ##  
           
           startTime <- Sys.time()
           print(paste("STARTING", testYear, target, "AT", startTime,"\n"))
           print(paste("STARTING", testYear, target, "AT", startTime,"\n"))
           print(paste("STARTING", testYear, target, "AT", startTime,"\n"))
           ...
           saveRDS(fit, here::here("Models", "Over Time", paste("Decision Tree vK0", testYear, target, "model.rds")))
