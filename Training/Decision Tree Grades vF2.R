# Decision Tree Grades vF2

# PURPOSE:  
# This runs four training models using regression and xgboost.
# It trains on only the first math class for first time freshman.
# It uses data derived by Bill Prisbrey.

# vF2 converts APCREDIT values of NA to zero, per e-mail convo with Luis--

# "AP credit data are ultimately at the student level, and I think we keep the focus here. We do not need to account for high schools when it comes to offering AP credits. For example, while we do get high school data, we cannot distinguish a student’s whole story here. We cannot know all of the opportunities that they had (or did not have) to engage in AP credits. Additionally, in this context it is not reasonable to do a deep dive into high schools to answer the following questions:

# •	Student attended multiple high schools, and had the opportunity to take AP in one but not the other. We only get the student’s last high school on record in many instances. 
# •	Student’s High School did not offer AP, but student had another opportunity through some other program to take them AP credits. 
...
cleanData <- firstCourses[cleanFilters & outlierFilters,]
...
lapply(target_classes,
  ...
  #     })
         ...
         saveRDS(theData, here::here("Data", paste("Decision Tree vF2", target, "Data.rds")))
      ...
      #   }
         ...
         saveRDS(fit, here::here("Models", paste("Decision Tree vF2", target, "model.rds")))
